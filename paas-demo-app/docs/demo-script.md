...
This script is a repeatable 3–5 minute walkthrough of the local PaaS control plane. The platform is the product; Deployment Lab is only the workload used to expose platform behavior.
...
- the Deployment Lab project uses port `5000` and healthcheck path `/health`;
...
APP_ENV=demo
APP_VERSION=1.0.0
APP_COMMIT_SHA=<current-demo-app-commit>
...
Open the Deployment Lab project and briefly show:
...
Select **Open service** and show Deployment Lab’s deployment receipt:
...
Refresh diagnostics and compare the new Pod name or open Deployment Lab again to show the changed hostname.
