...
Rejected examples include shell wrappers and shell control syntax such as `sh -c`, `bash -c`, `&&`, `||`, `|`, redirection, backticks, and subshells. The worker applies the same validation before running tests so an invalid command already present in the database fails at the `tests` step before `docker run` is invoked.
