...
const databaseHealth = ref("checking");
const databaseDetails = ref("Awaiting database probe");
...
const databaseStatusTone = computed(() => mapHealthTone(databaseHealth.value));
const databaseStatusLabel = computed(() => formatDatabaseLabel(databaseHealth.value));
...
async function fetchHealth() {
  ...
  try {
    const [serviceResponse, databaseResponse] = await Promise.all([
      fetch("/health"),
      fetch("/health/db"),
    ]);

    const serviceData = await readResponsePayload(serviceResponse);
    const databaseData = await readResponsePayload(databaseResponse);

    serviceHealth.value = serviceData?.status || "unknown";
    databaseHealth.value = databaseData?.database || databaseData?.status || "unknown";
    databaseDetails.value =
      databaseData?.details ||
      (databaseResponse.ok ? "SELECT 1 completed successfully" : `Health probe failed (${databaseResponse.status})`);
    lastHealthCheckAt.value = new Date().toISOString();
  } catch (error) {
    serviceHealth.value = "offline";
    databaseHealth.value = "offline";
    databaseDetails.value = error.message || "Health probe request failed";
    lastHealthCheckAt.value = new Date().toISOString();
  } finally {
...
<template>
  <main class="screen">
    ...
    <section class="hero">
      <p class="eyebrow">PAAS DEMO WORKLOAD</p>
    ...
    <section class="terminal-grid">
      <article class="terminal-panel">
        <div class="terminal-head health-head">
          <div>
            <span class="prompt prompt-green">paas_monitor@server:~ $</span>
        ...
        <div class="health-box">
          ...
          <p class="health-line">
            [ DATABASE:
            <strong :data-tone="databaseStatusTone">{{ databaseStatusLabel }}</strong>
            ]
          </p>
          <div class="divider"></div>
          <p class="log-line">...Service: <span class="ok-code">200 OK</span></p>
          <p class="log-line">
            ...DB Conn:
            <span :data-tone="databaseStatusTone">
              {{ databaseHealth === "reachable" ? "Connected" : "Failed" }}
            </span>
          </p>
          <p class="log-line health-details">...Last Check: {{ formatLastChecked(lastHealthCheckAt) }}</p>
          <p class="log-line health-details">...DB Detail: {{ databaseDetails }}</p>
        </div>
      </article>

      <article class="terminal-panel">
        <div class="terminal-head">
          <span class="prompt prompt-green">paas_deploy@server:~ $</span>
        ...
        <form class="terminal-form" @submit.prevent="createDeployment">
          <label class="terminal-row">
            ...
            <input v-model.trim="form.application_name" placeholder="[ billing-api ]" required />
          </label>
          <label class="terminal-row">
            <span class="row-label">&gt; Enter Version:</span>
            <input v-model.trim="form.version" placeholder="[ 2026.04.27-1 ]" required />
          </label>
    ...
    <section class="history-terminal">
      <div class="terminal-head history-head">
        <div>
          <span class="prompt prompt-green">paas_history@server:~ $</span>
