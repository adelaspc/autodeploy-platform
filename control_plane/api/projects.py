...
from control_plane.application.projects.service import (
...
from control_plane.application.deployments.read_models import (
    get_build_log_path,
...
from control_plane.application.projects.read_models import project_activity_payload, project_status_payload
from control_plane.api.live_health import probe_deployment_health
from control_plane.extensions import db
from control_plane.models import Build, DeploymentEvent, PlatformDeployment, Project
...
def protect_projects_api():
    # Individual write routes raise this baseline role with their own decorator.
...
def _queue_deployment_command(project_id, deployment, command_type, *, message=None):
    ...
    db.session.commit()
    record_audit_event(
        action=f"deployment.{command_type}_requested",
        resource_type="deployment",
        resource_id=deployment.id,
        metadata={
    ...
    payload = serialize_project_deployment(deployment, include_events=True)
...
def stop_project_deployment(project_id, deployment_id):
    ...
    _, validation_error = validate_deployment_patch_payload({"status": "stopped"}, deployment)
    if validation_error:
        return jsonify({"error": validation_error}), 400
    return _queue_deployment_command(project_id, deployment, "stop", message=message)
...
def cleanup_project_deployment(project_id, deployment_id):
    ...
    return _queue_deployment_command(project_id, deployment, "cleanup", message=message)
...
def update_project_deployment(project_id, deployment_id):
    ...
    if update_data.get("status") == "stopped":
        return _queue_deployment_command(
            project_id,
            deployment,
            "stop",
