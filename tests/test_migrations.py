    ...
    with app.app_context():
        ...
        db.session.execute(
            text(
                ...
                "VALUES (1, 'migration-project', 'https://example.invalid/repo.git', 'main', 'none', "
        ...
        db.session.execute(
            text(
                ...
                "(id, project_id, build_id, environment, status, created_at, updated_at) "
                "VALUES (1, 1, 1, 'production', 'pending', :now, :now)"
            ),
            {"now": now},
        )
        db.session.execute(
