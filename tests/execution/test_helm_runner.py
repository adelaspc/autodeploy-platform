...
from worker.helm.runner import HelmCommandError, HelmResult, HelmRunner
...
def test_kubeconfig_is_passed_to_all_helm_commands():
    runner = HelmRunner(namespace="apps", chart_path="chart", kubeconfig="/tmp/kubeconfig")

    for args in (
        runner.upgrade_install_args("release-name", "values.yaml"),
        runner.uninstall_args("release-name"),
        runner.status_args("release-name"),
    ):
        assert args[args.index("--kubeconfig") + 1] == "/tmp/kubeconfig"
