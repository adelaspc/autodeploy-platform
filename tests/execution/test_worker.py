...
def test_reconciler_cli_exposes_explicit_once_and_loop_commands(app):
    ...
    assert "run-reconciler-once" in commands
    assert "run-reconciler-loop" in commands
    assert "run-reconciler" in commands
...
def test_legacy_reconciler_cli_alias_remains_compatible(app):
    ...
    assert result.exit_code == 0
    assert "Reconciler finished with 0 action(s)" in result.output
