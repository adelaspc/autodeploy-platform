...
def test_frontend_fallback_serves_built_index(client, app):
    ...
    index_path.write_text("<!doctype html><title>AutoDeploy Control Plane</title>", encoding="utf-8")

    response = client.get("/")

    assert response.status_code == 200
    assert b"AutoDeploy Control Plane" in response.data
...
def test_frontend_fallback_does_not_serve_index_for_unknown_api_route(client, app):
    dist_dir = Path(app.static_folder)
    dist_dir.mkdir(parents=True, exist_ok=True)
    (dist_dir / "index.html").write_text("<!doctype html><title>AutoDeploy Control Plane</title>", encoding="utf-8")

    response = client.get("/api/not-real")

    assert response.status_code == 404
    assert b"AutoDeploy Control Plane" not in response.data
