...
AutoDeploy Control Plane is a local, operator-facing PaaS portfolio project. It manages projects, builds, deployments, background commands, deployment events, and reconciliation across Docker and Kubernetes runtimes.

## Repository Layout

```text
autodeploy-control-plane/
├── control_plane/         # API, application services, models, and persistence
├── worker/                # background execution and reconciliation
├── frontend/              # Vue operator console
├── migrations/
├── tests/
├── docs/
├── deploy/
...
- [Manual MicroK8s Validation](docs/microk8s-manual-validation.md)
...
The committed `.env*.example` files contain configuration templates only. Put passwords, API tokens, registry credentials, and Git tokens in the ignored `.env.secrets`; `make env-init` creates missing local files without overwriting existing ones, generates missing MySQL passwords, and applies mode `0600`.
...
The runtime image bundles the Helm CLI from the pinned `alpine/helm:4.2.0` build stage for Kubernetes Helm-mode deploy, status, reconciliation, and uninstall operations.
...
Configured executable commands such as `default_test_command` and deployment `test_command` are parsed as direct argv-style commands, not shell snippets. Simple commands such as `pytest -q`, `python -m pytest`, `python -m py_compile server.py`, `npm test`, and `ruff check .` are accepted. Shell wrappers and shell control syntax such as `sh -c`, `bash -c`, `&&`, `||`, `|`, redirection, backticks, and subshells are rejected at the API boundary and checked again by the worker. `migration_command` is validated and stored under the same command syntax rules for specification compatibility, but is reserved and is not executed or revalidated by the worker.
...
- `Bandit`: runs a targeted security scan against `control_plane/`, `worker/`, and `wsgi.py`
...
- `Compose Config`: validates the Docker Compose configuration
- `Helm Template`: renders the control-plane Helm chart with its CI values
- `Frontend`: installs locked dependencies, runs tests, and builds the Vue frontend on Node 22
...
- Docker build validation proves the application is still containerized and runnable
...
```bash
.venv/bin/flask --app wsgi:app run-reconciler-once
```
...
```bash
cd /path/to/autodeploy-control-plane
...
The API persists stop and cleanup requests as `DeploymentCommand` records and returns `202 Accepted`. The worker atomically claims each command and uses the executor recorded by `deploy_target`; HTTP request handlers never invoke Docker, Kubernetes, or Helm directly. In `local-docker` mode the worker captures container logs, removes the running container, clears `service_url`, and records `deployment.stop_started` and `deployment.stopped` events with cleanup metadata.
...
For a generic Kubernetes demo workload, application-owned variables can use:
